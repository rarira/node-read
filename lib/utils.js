...
var regexps = {
  ...
  positiveRe: /article|body|content|entry|hentry|page|pagination|post|text|cont_txt|aricle/i,
  negativeRe: /landing_boxㅣart_btm|news_like|news_rel|news_copyright_links|art_btm|list|twobox|photo|like|rel|links|emotion|table|arl_view|writer|date|combx|comment|captcha|contact|foot|footer|footnote|link|media|meta|promo|related|scroll|shoutbox|sponsor|utility|tags|widget|tip|dialog|title|ad|Ad|tit|subject/i,
  neverRe: /landing_boxㅣart_btm|news_like|news_rel|news_copyright_links/i,
...
function getCandidates($, base, options) {
  ...
  $('*', 'body').each(function(index, element) {
    ...
    // Resolve URLs
    if (base) {
      if (nodeType == 'img' && typeof node.attr('data-src') != 'undefined') {
    ...
    // if (
    //   nodeType == 'img' &&
    //   typeof node.attr('src') == 'undefined' &&
    //   typeof node.attr('data-src') == 'undefined'
    // ) {
    //   node.remove();
    // }
  });
...
function getClassWeight(node) {
  ...
  if (classAndID.search(regexps.neverRe) !== -1) weight -= 200;
  if (node.attr('itemprop') == 'articleBody') weight += 50;
