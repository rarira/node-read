...
var regexps = {
  unlikelyCandidatesRe: /sns|combx|pager|comment|disqus|foot|header|menu|meta|nav|rss|shoutbox|sidebar|sponsor|share|bookmark|social|advert|leaderboard|instapaper|unrelated/i,
  okMaybeItsACandidateRe: /and|article|body|column|main|figure|content|image|entry|src|post/i,
  positiveRe: /articleBody|news_text|news_content|ab|caption|interview|question|answer|news_img|image|photo|section-txt|article|body|content|entry|hentry|page|pagination|post|text|cont_txt|aricle|image_src/i,
  negativeRe: /landingㅣbtm|like|rel|copyright|art_btm|twobox|like|rel|links|emotion|table|arl_view|writer|date|combx|comment|captcha|contact|foot|footer|footnote|link|media|meta|promo|related|scroll|shoutbox|sponsor|utility|tags|widget|tip|dialog|title|ad|Ad|tit|subject/i,
  neverRe: /sns|widget-inline-editors-picks|news_like|news_tag|news_rel|news_copyright_links|best_nw|Ad|tit|extra|landingㅣbtm|editor|profile|like|rel|copyright/i,
  divToPElementsRe: /<(a|figure|post|blockquote|dl|div|img|ol|p|pre|table|ul)/i,
  replaceBrsRe: /(<br[^>]*>[ \n\r\t]*){2,}/gi,
...
var nodeTypes = {
  mostPositive: ['div'],
  positive: ['pre', 'td', 'blockquote', 'figure'],
...
function filterCandidates(topCandidate, siblings, $) {
  ...
  var siblingScoreThreshold = Math.max(
    10,
    topCandidate.data('readabilityScore') * 0.2
  );
...
function getCandidates($, base, options) {
  ...
  $('*', 'body').each(function(index, element) {
    ...
    if (
      classAndID.search(regexps.unlikelyCandidatesRe) !== -1 &&
    ...
    if (
      nodeType == 'img' &&
      typeof node.attr('src') == 'null' &&
      typeof node.data('src') == 'null'
    ) {
      node.remove();
    }
  });
...
function getClassWeight(node) {
  ...
  if (node.get(0).name === 'article' || node.attr('itemprop') === 'articleBody')
    weight += 25;
  if (classAndID.search(regexps.negativeRe) !== -1) weight -= 25;
  if (classAndID.search(regexps.positiveRe) !== -1) weight += 25;
  // if (node.attr('class') === 'ab_photo photo_center ') weight += 100;
  if (classAndID.search(regexps.neverRe) !== -1) {
    node.remove();
  }
